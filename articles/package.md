...
[`mold()`](https://hardhat.tidymodels.org/reference/mold.md) and
[`forge()`](https://hardhat.tidymodels.org/reference/forge.md) in a
...
[`mold()`](https://hardhat.tidymodels.org/reference/mold.md) and
[`forge()`](https://hardhat.tidymodels.org/reference/forge.md) fit in
the bigger picture, rather than trying to understand how the model
works.
...
[`mold()`](https://hardhat.tidymodels.org/reference/mold.md) will
...
[`mold()`](https://hardhat.tidymodels.org/reference/mold.md), here I’ve
called that `processed`. In that object will be (at a minimum) the
...
from [`mold()`](https://hardhat.tidymodels.org/reference/mold.md) will
*always* be data frames, so in this case we can convert them to matrices
and vectors directly for use in the lower level function.
...
[`mold()`](https://hardhat.tidymodels.org/reference/mold.md) with the
method specific inputs to run the preprocessing, and then pass off to
the bridge function to run the actual model. It is also good practice to
provide a default method with a nice error message for unknown types.
...
[`mold()`](https://hardhat.tidymodels.org/reference/mold.md).
...
recap, by default
[`mold()`](https://hardhat.tidymodels.org/reference/mold.md) will *not*
automatically add an intercept for any method, including the formula
method.
...
[`forge()`](https://hardhat.tidymodels.org/reference/forge.md) on the
output manually. The higher level user facing function will do this
automatically.
...
value of a call to
[`forge()`](https://hardhat.tidymodels.org/reference/forge.md). Since
the prediction implementation function takes a matrix, we convert it to
that here.
...
call [`forge()`](https://hardhat.tidymodels.org/reference/forge.md) with
the `new_data` and the `blueprint` that we attached to our `simple_lm`
model at fit time. This performs the required preprocessing on the new
data, and checks that the *type* of the data frame supplied to
`new_data` matches the type of data frame supplied at fit time. This is
one of the most valuable features of
[`forge()`](https://hardhat.tidymodels.org/reference/forge.md), as it
adds a large amount of robustness to your
[`predict()`](https://rdrr.io/r/stats/predict.html)ion function. You
will see some examples of this at the end of the vignette.

After calling
[`forge()`](https://hardhat.tidymodels.org/reference/forge.md), it
...
By using [`forge()`](https://hardhat.tidymodels.org/reference/forge.md),
you automatically get powerful type checking to ensure that the
`new_data` is in a form that you expect.
