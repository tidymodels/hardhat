...
[`mold()`](https://hardhat.tidymodels.org/reference/mold.md) is called
using the xy-method, and a *vector* `y` value is supplied rather than a
data frame or matrix. In that case, `y` is coerced to a data frame, and
the automatic name `".outcome"` is added, and this is what is looked for
in [`forge()`](https://hardhat.tidymodels.org/reference/forge.md). If
