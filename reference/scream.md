...
`scream()` is called by
[`forge()`](https://hardhat.tidymodels.org/reference/forge.md) after
[`shrink()`](https://hardhat.tidymodels.org/reference/shrink.md) but
before the actual processing is done. Generally, you don't need to call
`scream()` directly, as
[`forge()`](https://hardhat.tidymodels.org/reference/forge.md) will do
it for you.

If `scream()` is used as a standalone function, it is good practice to
call [`shrink()`](https://hardhat.tidymodels.org/reference/shrink.md)
