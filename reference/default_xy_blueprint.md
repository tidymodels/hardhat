...
[`mold()`](https://hardhat.tidymodels.org/reference/mold.md) if `x` and
`y` are provided separately (i.e. the XY interface is used).
...
[`mold()`](https://hardhat.tidymodels.org/reference/mold.md). In that
case, [`mold()`](https://hardhat.tidymodels.org/reference/mold.md)
converts `y` into a tibble, with a default name of `.outcome`. This is
the column that
[`forge()`](https://hardhat.tidymodels.org/reference/forge.md) will look
