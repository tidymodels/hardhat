...
#'
#' # Occasionally, you might have a custom role that is only required at
#' # `prep()` time, but not at `bake()` time. To handle this, you'll want
#' # `forge()` to ignore columns with that role when it does structural checks
#' # on `new_data`. You can specify these types of roles with the
#' # `prep_specific_roles` argument to `default_recipe_blueprint()`.
#' bp <- default_recipe_blueprint(prep_specific_roles = "custom_role")
#'
#' processed_roles <- mold(rec_roles, train, blueprint = bp)
#'
#' # The `"custom_role"` will still be in the `mold()` result
#' processed_roles$extras$roles$custom_role
#'
#' # But it won't be required in `forge()`,
#' # and will be excluded from the result there
#' test2 <- test
...
mold_recipe_default_process_extras <- function(blueprint, data) {
  ...
  # Capture original non standard role columns
  # (anything but "predictor", "outcome", or `prep_specific_roles`).
  # These are required in `new_data`.
  original_extra_role_cols <- get_extra_role_columns_original(
    blueprint$recipe,
    data,
    blueprint$prep_specific_roles
  )
  ...
  # These include `prep_specific_roles`, assuming the developer may need
  # them for modeling.
...
forge_recipe_default_process_extras <- function(extras,
  ...
  # Get the non standard role columns after `bake()` has been called.
  # This excludes "outcome", "predictor", and `prep_specific_roles` roles.
...
get_extra_role_columns_original <- function(rec, data, prep_specific_roles) {
  # Extra roles that existed before `prep()` has been called.
  # Their ptypes are retained and required in `new_data` unless `prep_specific_roles` excludes them.
  get_extra_role_columns(rec, data, prep_specific_roles = prep_specific_roles, info_type = "var_info")
}
get_extra_role_columns_mold <- function(rec, data) {
  ...
  get_extra_role_columns(rec, data, prep_specific_roles = character(), info_type = "term_info")
}
get_extra_role_columns_forge <- function(rec, data, prep_specific_roles) {
  # Extra roles that exist after `bake()` has been called on the test data.
  # `data` won't include any columns corresponding to `prep_specific_roles`,
  # but we still need to pass that info through to remove them from the role info in `rec`.
  get_extra_role_columns(rec, data, prep_specific_roles = prep_specific_roles, info_type = "term_info")
}

get_extra_role_columns <- function(rec, data, prep_specific_roles, info_type) {
  roles <- rec[[info_type]][["role"]]
  vars <- rec[[info_type]][["variable"]]

  extra_roles <- setdiff(roles, c("predictor", "outcome", NA_character_, prep_specific_roles))
