...
#' @rdname validation
validate_is <- function(x, x_nm, is_cls, cls) {
  if (!is_cls(x)) {
    glubort("`{x_nm}` must be a {cls}, not a {class(x)[1]}.")
  }
}
...
#' @rdname validation
validate_named <- function(x, x_nm) {
  
  x_nms <- names(x)
  
  if (length(x_nms) != length(x)) {
    glubort("All elements of `{x_nm}` must be named.")
  }
  
  if (any(is.na(x_nms) | x_nms == "")) {
    glubort("No names of {x_nm} may be `NA` or the empty string, i.e. ''. ")
  }
  
  if (anyDuplicated(x_nms)) {
    glubort("All elements of `{x_nm}` must have unique names.")
  }
