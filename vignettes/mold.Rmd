...
_Note:_ It's worth mentioning that when an intercept is not present, base R expands factors columns completely (also known as one hot encoding) into `K` columns corresponding to the `K` levels present in the _first_ factor and `K-1` for the rest. When an intercept is present, only `K-1` columns are generated for all factor predictors.
