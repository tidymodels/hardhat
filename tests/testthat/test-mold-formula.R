...
test_that("factor predictors with no intercept are fully expanded", {
  ...
  x <- mold(Sepal.Length ~ Species, iris, engine = default_formula_engine(intercept = TRUE))
  xx <- mold(Sepal.Length ~ Species, iris, engine = default_formula_engine(intercept = FALSE))
...
test_that("errors are thrown if `indicator = FALSE` and factor interactions exist", {
  ...
  expect_error(
    mold(~ Species, iris, engine = default_formula_engine(indicators = FALSE)),
    NA
  )

  expect_error(
    mold(Sepal.Length ~ Species:Sepal.Width, iris, engine = default_formula_engine(indicators = FALSE)),
  ...
  expect_error(
    mold(Sepal.Length ~ (Species + Sepal.Width) ^ 2, iris, engine = default_formula_engine(indicators = FALSE)),
    "'Species'"
  )

  expect_error(
    mold(Sepal.Length ~ Species %in% Sepal.Width, iris, engine = default_formula_engine(indicators = FALSE)),
    "'Species'"
  )
  ...
  expect_error(
    mold(~ Species:Species2, iris2, engine = default_formula_engine(indicators = FALSE)),
    "'Species', 'Species2'."
  )
...
test_that("errors are thrown if `indicator = FALSE` and factors are used in inline functions", {
  ...
  expect_error(
    mold(~ paste0(Species), iris, engine = engine_no_indicators),
  ...
  expect_error(
    mold(~ Species %>% paste0(), iris, engine = engine_no_indicators)
  )

  expect_error(
    mold(~ paste0(Species + Species), iris, engine = engine_no_indicators),
    "'Species'"
  )
  ...
  expect_error(
    mold(~ paste0(Species) + paste0(Species2), iris2, engine = engine_no_indicators),
    "'Species', 'Species2'."
  )
...
test_that("formula intercepts can be added", {
  ...
  expect_false("(Intercept)" %in% colnames(x$engine$ptypes$predictors))
})
...
test_that("`indicators = FALSE` runs numeric interactions", {
  ...
  x <- mold(~ Sepal.Length:Sepal.Width, iris, engine = default_formula_engine(indicators = FALSE))
...
test_that("`.` notation with variable as predictor and outcome", {
  ...
  expect_true(
    "Sepal.Width" %in% colnames(x$engine$ptypes$predictors)
  )
...
test_that("`-var` still registers var as a predictor", {
  ...
  # Sepal.Length IS a predictor
  expect_true(
    "Sepal.Length" %in% colnames(x$engine$ptypes$predictors)
  )
