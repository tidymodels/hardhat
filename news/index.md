...
- [`mold()`](https://hardhat.tidymodels.org/reference/mold.md) no longer
  throws warnings about `strings_as_factors` argument when used on
  recipe objects
...
- Using a formula blueprint with `indicators = "traditional"` or
  ...
  during [`forge()`](https://hardhat.tidymodels.org/reference/forge.md)
  ([\#213](https://github.com/tidymodels/hardhat/issues/213)).
...
- New internal
  [`recompose()`](https://hardhat.tidymodels.org/reference/recompose.md)
...
- We have reverted the change made in hardhat 1.0.0 that caused recipe
  ...
  - By default,
    [`forge()`](https://hardhat.tidymodels.org/reference/forge.md) will
    pass on all columns from `new_data` to
  ...
  - In the development version of recipes, which will become recipes
    ...
    won’t pass on columns of `new_data` to
...
- Fixed a bug where the results from calling
  [`mold()`](https://hardhat.tidymodels.org/reference/mold.md) using
  hardhat \< 1.0.0 were no longer compatible with calling
  [`forge()`](https://hardhat.tidymodels.org/reference/forge.md) in
...
- Internal details related to how blueprints work alongside
  [`mold()`](https://hardhat.tidymodels.org/reference/mold.md) and
  [`forge()`](https://hardhat.tidymodels.org/reference/forge.md) were
  heavily re-factored to support the fix for
  ...
  - Blueprints no longer store the clean/process functions used when
    calling [`mold()`](https://hardhat.tidymodels.org/reference/mold.md)
    and [`forge()`](https://hardhat.tidymodels.org/reference/forge.md).
...
- Moved [`tune()`](https://hardhat.tidymodels.org/reference/tune.md)
...
- [`mold()`](https://hardhat.tidymodels.org/reference/mold.md) no longer
  misinterprets `::` as an interaction term
  ([\#174](https://github.com/tidymodels/hardhat/issues/174)).

- When `indicators = "none"`,
  [`mold()`](https://hardhat.tidymodels.org/reference/mold.md) no longer
  misinterprets factor columns as being part of an inline function if
  there is a similarly named non-factor column also present
...
- [`scream()`](https://hardhat.tidymodels.org/reference/scream.md) is
  now stricter when checking ordered factor levels in new data against
  the `ptype` used at training time. Ordered factors must now have
  *exactly* the same set of levels at training and prediction time. See
  [`?scream`](https://hardhat.tidymodels.org/reference/scream.md) for a
  new graphic outlining how factor levels are handled
  ([\#132](https://github.com/tidymodels/hardhat/issues/132)).

- The novel factor level check in
  [`scream()`](https://hardhat.tidymodels.org/reference/scream.md) no
  longer throws a novel level warning on `NA` values
...
- [`forge()`](https://hardhat.tidymodels.org/reference/forge.md) now
